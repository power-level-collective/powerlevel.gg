 ...
 * chip when there's no icon to show instead. `size="lg"` is a bigger variant for standalone use
 * (e.g. the homepage capabilities panel) instead of an inline credit row.
 */
export function IconBadge({
    item,
    size = "sm",
}: {
    item: { name: string; icon?: string; logo?: string; url?: string };
    size?: "sm" | "lg";
}) {
    ...
    const boxClass = size === "lg" ? "h-14 w-14 p-2.5" : "h-8 w-8 p-1.5";
    const tooltipClass = size === "lg" ? "-top-9" : "-top-8";

    return (
        <Wrapper
            {...wrapperProps}
            className={`group relative flex ${boxClass} shrink-0 items-center justify-center rounded bg-surface-2 shadow-[inset_0_0_0_1px_var(--color-border)] transition hover:bg-surface`}
        >
            <img src={src} alt={item.name} className="h-full w-full object-contain" />
            <span
                className={`pointer-events-none absolute ${tooltipClass} left-1/2 z-10 -translate-x-1/2 whitespace-nowrap rounded bg-bg-elevated px-2 py-1 text-[0.65rem] font-bold uppercase tracking-wide text-ink opacity-0 shadow-[inset_0_0_0_1px_var(--color-border-strong)] transition-opacity duration-150 group-hover:opacity-100 group-focus-visible:opacity-100`}
            >
                {item.name}
            </span>
        </Wrapper>
...
export function CreditBadge({ credit, size = "sm" }: { credit: Studio; size?: "sm" | "lg" }) {
    const Wrapper = credit.url ? "a" : "div";
    const wrapperProps = credit.url ? { href: credit.url, target: "_blank", rel: "noopener noreferrer" } : {};
    const minHeight = size === "lg" ? "min-h-40 sm:min-h-48" : "min-h-24";
    const imageHeight = size === "lg" ? "max-h-28 sm:max-h-32" : "max-h-16";

    if (credit.logo) {
        return (
            <Wrapper
                {...wrapperProps}
                className={`panel-cut-sm group relative flex ${minHeight} items-center justify-center bg-surface-2 px-6 py-4 transition hover:bg-surface`}
            >
                <img src={credit.logo} alt={credit.name} className={`${imageHeight} max-w-full object-contain`} />
    ...
    return (
        <Wrapper
            {...wrapperProps}
            className={`panel-cut-sm flex ${minHeight} items-center justify-center bg-surface-2 px-6 py-4 text-center transition hover:bg-surface`}
        >
